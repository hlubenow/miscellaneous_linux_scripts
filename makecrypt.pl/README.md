...
`makecrypt.pl` uses system-calls to the shell commands described in [my article](https://hlubenow.lima-city.de/suse131.html#30). Maybe it would be a good idea to go through the article and try to setup a data safe by hand first (checking if all necessary shell commands are available), before trying to do it by script.
