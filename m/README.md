...
Passing a number to it, makes it try to play that and the following files: That is, if `14` is passed, and it finds at least `14.mp3`, it tries to play `14.mp3`, `15.mp3` and so on. It may prefer `.mp4`, if it finds both `.mp3` and `.mp4` in one directory (I may fix that one day.)
